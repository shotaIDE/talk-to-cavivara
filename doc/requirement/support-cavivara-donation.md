...
ユーザーがカヴィヴァラさんたちを「応援する」という体で、金銭的なサポート（投げ銭・寄付）を行える機能を提供する。この機能は、既存のPro版アップグレードとは別の課金形態として実装され、感謝の気持ちを表現する手段として位置付けられる。

## 背景

現在、アプリには買い切り型のPro版アップグレード機能が存在する。しかし、以下の理由から、追加の課金オプションが必要である:

- Pro版を既に購入済みのユーザーが追加でサポートする手段がない
- 機能的な対価ではなく、純粋に「応援したい」という気持ちを表現したいユーザーのニーズがある
- 継続的な開発・運営コストをサポートする仕組みが必要
- カヴィヴァラキャラクターへの愛着を行動に移す場が求められている

「カヴィヴァラを応援する」という表現を用いることで、課金に対する心理的なハードルを下げ、ポジティブな体験として提供する。
...
- 課金は都度課金（非消費型）として実装される
- Pro版未購入のユーザーも応援課金が可能
...
ユーザーとして、Pro版アップグレードと応援課金の違いを明確に理解したい。なぜならば、自分のニーズに合った課金方法を選びたいからだ。

受入基準:

- 応援画面にPro版との違いが明記される
  - Pro版: 機能アンロック（買い切り）
  - 応援課金: 純粋なサポート、機能追加なし（都度課金）
- 「応援課金では機能は追加されませんが、開発を支援できます」などの説明
- Pro版未購入の場合、Pro版へのリンクも提供（相互補完）
...
- [Pro版アップグレード画面](../../client/lib/ui/feature/pro/upgrade_to_pro_screen.dart)
